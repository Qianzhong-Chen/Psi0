...
# Fine-tune Psi0 with 23-D upper-body-joint actions on the universal column
# schema shared by psi0 / pi05 / gr00t-n1.6.
# Usage: ./finetune-real-psi0-23act-upperjoint.sh [task] [exp]
#
# Action (23): L_arm(7) + R_arm(7) + waist_rpy(3) + L_grip(1) + R_grip(1)
#              + height(1) + torso_vx + torso_vy + torso_vyaw
# State  (20): L_arm(7) + R_arm(7) + waist_rpy(3) + L_grip(1) + R_grip(1)
#              + height(1)
#
# Deploy path: VLA → 23-D joint targets → low-level joint tracker
# Source columns: observation.state_uni_20 / action.uni_23 — built by
# scripts/data/add_uni_23act_columns.py (joints/rpy sourced from
# observation.state[t+1]; see docstring for the commanded-vs-observed
# rationale around wigs+teleop action-distribution parity).
...
DEFAULT_REPO="hojjunekim/wigs_locomani_adjust_uni"
export task="${1:-$DEFAULT_REPO}"
export exp="${2:-psi0-23act-upperjoint}"
